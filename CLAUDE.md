...
- **Generator mapowań przez LLM** (`profile-generator.ts` + `llm-client.ts`): do API AI trafiają WYŁĄCZNIE nagłówki + zredagowana próbka (`sample-redactor.ts`), za jawną zgodą użytkownika w kreatorze; profil waliduje zod + deterministyczny self-check na realnym pliku (próg pewności 0.8, max 2 retry); wynik zapisywany jako `pending` (generatedBy='llm' + model + confidence). Env:
  ...
  - `LLM_MODEL` (domyślnie `mistral-small-latest`), `LLM_TIMEOUT_MS` (domyślnie 60000)
