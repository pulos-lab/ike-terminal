...
- **Sufiks giełdy bije etykietę waluty.** Pseudo-ISIN z kanonicznym sufiksem Yahoo (`SMSN.L`, `INPST.AS`, `NOVOB.CO` — zbiór w `shared/src/yahoo-exchange-suffix.ts`, parytet z `XTB_TO_YAHOO` pilnuje test) NIE jest polski, choćby transakcja była opisana jako PLN. Taki symbol jest sprawdzany WPROST (`fetchYahooPrice` + `fetchYahooSymbolInfo`, ta sama ścieżka co ręczne dodanie tickera), a nie przez wyszukiwarkę: dla `SMSN.L` (GDR Samsunga na LSE, notowany w USD) `search` podsuwa `SMSN.IL`, więc trafienie nie jest dokładne i guard waluty (GBP z sufiksu `.UK` vs USD z notowania) odrzucał je — pozycja zostawała nierozpoznana. Waluta i nazwa idą z odpowiedzi Yahoo; `exchange` zostaje `OTHER` (LSE nie jest w unii — świadomie, patrz `resolveEarningsMarket`), więc nazwę trzeba ustawić OD RAZU
