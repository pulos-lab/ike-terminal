...
### Splity — detekcja i korekta ilości
Trzy niezależne tory w `split-detector.ts`, scalane w `mergeDetectedSplits`: (1) cena transakcji vs cena providera z dnia transakcji, (2) sprzedaż przekraczająca zakumulowane kupno, (3) **skok w samej serii providera**. Realne daty i ratio nadaje `resolveSplitEventDates` z eventów Yahoo.
- **Nie każdy provider koryguje historię.** Domyślne założenie całego silnika brzmi „seria jest już skorygowana o splity" — i przy takiej serii tor (1) działa. Invesco S&P 500 UCITS ETF (`P500.DE`, split 100:1 z 2025-12-15) pokazał, że Yahoo potrafi NIE mieć zdarzenia (`events` puste) i NIE skorygować historii (`adjclose == close`, w serii surowy skok 1157,40 → 11,57). Wtedy tor (1) porównuje dwie liczby w tej samej starej skali, ratio ≈ 1 i nie wykrywa nic
- **Tor (3) — `detectSplitsFromPriceSeries`** — łata tę dziurę, ale nie ma czym potwierdzić wykrycia (gdyby Yahoo znało zdarzenie, seria byłaby skorygowana), więc progi są ostrzejsze: minimum 10× (przy 2× każdy krach −50% to „split 2:1"), snap do znanego ratio w 2%, maksymalna przerwa 7 dni między punktami, oba poziomy muszą się utrzymać przez 2 punkty. Wyniki NIE idą przez `resolveSplitEventDates` — data ze skoku JEST dokładną ex-datą
- **Split jest TRWAŁY — poziom nie wraca.** Skok, po którym stary poziom pojawia się ponownie (albo nowy istniał wcześniej), to rozjazd JEDNOSTKI u providera: pensy vs funty na listingu londyńskim. Zmierzone na całej historii cen produkcji (910 serii, 990 tys. punktów): jedyne fałszywki to `LTSV.L` skaczący 2564 → 25,70 i z powrotem, cztery razy. Sprawdzenie musi być symetryczne — skok powrotny bywa niewykryty, bo odpada na progu stabilności
- **`normalizeUnadjustedProviderSeries`** dokańcza za providera korektę: dzieli odcinek serii sprzed ex-date przez ratio. Bez tego `adjustTransactionsForSplits` (ilość ×ratio) mnożyłoby się z ceną w starej skali i wycena historyczna byłaby `ratio` razy ZA WYSOKA. „Seria jest surowa" liczymy na żywo z obecności skoku, nie utrwalamy flagą — gdy provider poprawi dane, normalizacja sama przestanie się stosować
- **REGUŁA OGÓLNA: iloraz cen sam z siebie nie rozstrzyga.** 3,93 to zły papier ALBO split 4:1 (`ATAL`→`ATA.WA` vs `IBKR` po splicie — nieodróżnialne). Skok 100× to split ALBO pensy/funty. −50% to krach ALBO split 2:1. Za każdym razem rozstrzyga fakt spoza ceny: zdarzenie w `stock_splits`, trwałość poziomu, zapis w bazie
...
- **Dwie mapy aliasów, dwa różne byty.** `FOREIGN_TICKER_ALIASES` = rebranding w czasie (symbol był poprawny i się zmienił, `RELI`→`EZRA`). `BROKER_TICKER_ALIASES` = notacja brokera (symbol NIGDY nie był poprawny u nas): Yahoo zapisuje klasę akcji z MYŚLNIKIEM, XTB skleja — `BRKB`→`BRK-B`, `BFB`→`BF-B`, `NOVOB`→`NOVO-B.CO`, a klasa C Alphabetu siedzi u Yahoo pod gołym `GOOG` (`GOOGC`→`GOOG`). Wpisy są ręczne, bo automat tu nie działa: dla `GOOGC` poprawnego papieru NIE MA w wynikach wyszukiwarki, `ALPHABET INC C` zwraca same cross-listingi, a `Berkshire Hathaway B` podaje jako pierwszy lewarowany ETP
- **Guard waluty dla trafień NIE-dokładnych** (`isExactSymbolMatch`): `isPlausibleMatch` celowo dopuszcza różnicę sufiksu giełdy (`BRKB`→`BRKB.VI`), ale tą samą furtką wchodzi cudzy listing. Gdy symbol nie jest dokładnym trafieniem, żądamy zgodności waluty notowania z walutą transakcji. Zakres jak przy `pickPlausible` (pseudo-ISIN, niepolski, nie-CFD) — pomiar na 261 zastanych wpisach w zakresie: guard odrzuciłby dokładnie 1, ten błędny
- **PUŁAPKA: zła etykieta waluty wpycha obcy papier w polską gałąź.** `isPolishTicker` jest prawdziwe dla `isPseudoIsin && txCurrency === 'PLN'`, a XTB potrafi zapisać walutę KONTA zamiast notowania. Amerykańska spółka trafia wtedy w gałąź akceptującą wyłącznie `.WA` i skracanie nazwy dorabia jej polski odpowiednik (`INTU` → `INT.WA` Internity; cena 551,07 „PLN" to w rzeczywistości 545,40 USD). Kolizja tickera jest SKUTKIEM, przyczyną jest etykieta
- **Samoleczenie obejmuje OBIE klasy**: brak wpisu ORAZ prowizoryczny stub (`getUnresolvedTickerIsins`). Wcześniej pass patrzył tylko na stuby, a stuba zapisuje wyłącznie ścieżka CSV — import binarny (XTB XLSX, IBKR HTML) idzie przez `importCombinedFiles`, który po nierozpoznanym papierze nie zostawia ŻADNEGO wpisu, więc pozycje z tych brokerów nie goiły się nigdy
- **Katalog BR musi być gotowy PRZED rozpoznawaniem.** `ensureIndex` przy pustym katalogu czeka najwyżej `COLD_START_WAIT_MS` (2,5 s) i zwraca pustą listę, a `findByTicker`/`findByName` NIE odróżniają „jeszcze nie wczytane" od „nie ma takiej spółki" (`search()` ma fallback na statyczną mapę NC, one nie). Pobranie ~1,2 MB regularnie przekracza ten limit, więc `resolveUnknownIsins` woła `warmUp()` (czeka bez limitu, no-op gdy katalog używalny), a `index.ts` rozgrzewa katalog 3 s po starcie i co 12 h. Objaw bez tego: 7 spółek z pierwszej ligi GPW bez ceny mimo poprawnych danych w katalogu
- **KOD TICKERA BYWA RECYKLOWANY** — po delistingu GPW/NC zwalnia trzyliterowy kod i nadaje go innej spółce. Alias rebrandingowy, którego KLUCZ jest dziś żywym kodem, kieruje pozycję na CUDZY papier: `SUN → MIG` (Sundragon→Military Group, 2025) rozjechał Suntech, który przejął kod `SUN`. Stąd dwie zmiany w gałęzi polskiej: (1) gdy mapa NC potwierdza, że kod jest DZIŚ czyimś tickerem, pytamy katalog BR o niego SUROWO przed aliasem (bez tego dowodu kolejność zostaje dawna — przy rebrandingu stary kod jest martwy i alias jest jedyną drogą), (2) **DOKŁADNY kod w statycznej mapie NC rozstrzyga przed dopasowaniem po NAZWIE**. `STOOQ_ALIASES` pilnuje test `isin-resolver-alias-hygiene.test.ts` (klucz nie może być tickerem żywym w `NC_TICKER_MAP`; jedyny wyjątek `7FT`, gdzie nieaktualny jest wpis mapy NC, nie alias)
- **Prefiks nazwy poniżej 4 znaków to loteria** — `findByName` w katalogu BR rozstrzygał remis kolejnością w indeksie, więc `SUN` łapało `SUNNET` (SNN leży przed SUN). Dopasowanie prefiksowe działa od 4 znaków, a remis (≥2 wpisy z tym samym najlepszym score) zwraca `null`; dokładna równość skrótu jest bez progu
- **Sufiks giełdy bije etykietę waluty.** Pseudo-ISIN z kanonicznym sufiksem Yahoo (`SMSN.L`, `INPST.AS`, `NOVOB.CO` — zbiór w `shared/src/yahoo-exchange-suffix.ts`, parytet z `XTB_TO_YAHOO` pilnuje test) NIE jest polski, choćby transakcja była opisana jako PLN (bywa nią waluta KONTA). Waluta notowania idzie z odpowiedzi Yahoo, nie z sufiksu kraju: Londyn kwotuje GDR-y i część klas ETF-ów w USD (`SMSN.IL`, `ISAC.L`), więc dla trafienia różniącego się WYŁĄCZNIE sufiksem giełdy guard waluty ustępuje. Dla symbolu BEZ sufiksu (`BRKB` → `BRKB.VI`) etykieta pochodzi z pliku brokera i guard działa jak dotąd. `exchange` zostaje `OTHER` (LSE nie jest w unii — świadomie, patrz `resolveEarningsMarket`), więc nazwę trzeba ustawić OD RAZU
- **CENA Z CHARTU NIE JEST DOWODEM, ŻE SYMBOL ISTNIEJE.** Mapa sufiksów XTB→Yahoo jest mechaniczna (`.UK → .L`), a Londyn ma dwie linie: zwykłe akcje (`VOD.L`) i międzynarodowy order book (`SMSN.IL`). Dla `SMSN.UK` chart API oddawało cenę na `SMSN.L` — ~4× niższą od instrumentu, który XTB faktycznie śledzi (zakup z pliku po 4100, notowanie 1179,5) — a wyszukiwarka tego symbolu NIE ZNAŁA. Dlatego bezpośrednie sprawdzenie symbolu zaczyna się od `fetchYahooSymbolInfo` (dokładna zgodność symbolu = potwierdzenie, że papier istnieje) i dopiero potem pyta o cenę; brak potwierdzenia → szukamy po SAMYM KODZIE (`SMSN` → `SMSN.IL`), tak jak przy ręcznym dodaniu tickera
- **Nazwa równa tickerowi = wpis NIEPOTWIERDZONY** (`isProvisionalStub`, także dla symbolu z kropką — dawniej wykluczonego). Bez tego zły wpis `SMSN.L` z nazwą „SMSN.L" zamarzał pod kotwicą na zawsze, bo self-heal patrzył wyłącznie na symbole bez sufiksu; ponowne rozpoznanie idzie przez te same guardy, więc kosztuje jeden lookup, a ratuje pozycję bez akcji użytkownika
- **Skracanie nazwy do 3-4 znaków jest bezpieczne dzięki `namesOverlap`** — kandydat obcięty przechodzi tylko przy zgodności 4-znakowego prefiksu nazwy (`MIRB` ≠ `MIRA`, więc `MIRBUD` nie złapie `MIRACULUM`). Wpisy sprzed tej bramki zostały w bazie, bo chroni je kotwica; naprawia je `npm run fix:ticker-price-mismatch -w server`
...
- **ZMERGOWANE ≠ NA PRODUKCJI.** Na serwer wynosi kod wyłącznie workflow `deploy.yml`, wyzwalany pushem do `main`. Gdy GitHub Actions ma awarię, merge się udaje, ale run deploya NIE POWSTAJE — a GitHub nie nadrabia przegapionych zdarzeń i nie ma czego „powtórzyć z UI" (workflow nie ma `workflow_dispatch`). Zdarzyło się 2026-08-06: pięć PR-ów wylądowało w `main`, produkcja została na poprzednim commicie. Weryfikacja idzie z SERWERA, nie z GitHuba: data `server/dist/index.js` + `grep` po nazwie nowej funkcji w `dist`. Odblokowanie: jakikolwiek nowy push do `main` (deploy buduje z aktualnego HEAD, więc jeden push dowozi całą zaległość)
...
- **Test dotykający bazy MUSI izolować `DATA_DIR`** — i to przez `process.env.DATA_DIR` na górze pliku PLUS **dynamiczny** import modułu testowanego. Statyczne importy w ESM wykonują się PRZED ciałem modułu, więc `config.dataDir` zdąży się policzyć ze ścieżki domyślnej i test zapisze do prawdziwego `data/` repo. Zaobserwowane przy pisaniu testu `resolveUnknownIsins`: wyciek implementacji mocka między testami zostawił tam wpis `ALLEGRO → ABE.WA (AB S.A.)` — dokładnie taki błędny wpis, jakiego potem broni kotwica
...
- `npm run fix:mismatched-tickers -w server` — naprawa wpisów `ticker_map` wskazujących CUDZĄ spółkę, wykrytych walidatorem `isPlausibleMatch` (dry-run; `-- --apply` zapisuje, `-- --include-closed` obejmuje pozycje zamknięte). Tym idzie naprawa zastanych `SUN.WA → SNN.WA`; na produkcji przez workflow `fix-tickers-prod.yml` (input `run_mismatched`)
- `npm run fix:ticker-price-mismatch -w server` — jednorazowa naprawa wpisów `ticker_map` wskazujących CUDZY papier (dry-run; `-- --apply` zapisuje, `-- --only=ID` zawęża). Kandydat musi przejść OBA sita: walidację trafienia i rozjazd ceny transakcji vs notowanie — sam przesiew po niezgodności WALUTY jest bezwartościowy (30 z 31 takich wpisów to poprawne cross-listingi). Obchodzi kotwicę `upsertTickerMapEntry` przez `force`, więc uruchamiać świadomie i po przejrzeniu dry-runu
