...
- "classify": ordered rules, first match wins: {"id","when":[matchers ANDed],"emit":<rowClass>}. Matcher: {"col":<ref>,"op":"equals"|"oneOf"|"contains"|"startsWith"|"regex"|"signPositive"|"signNegative"|"zeroNumber"|"nonzeroNumber"|"empty"|"notEmpty","values"?:[…],"pattern"?:"…"}. Row classes: trade, dividend, withholding_tax, coupon, interest, deposit, withdrawal, fx_leg, fee, trade_fee, commission_refund, capital_return, share_in, other, skip. "defaultClass": "skip" (unmatched rows reported) or "other" (imported as misc cashflow).
- Every emitted class needs its mapping section: "trade" for trades; cash classes use keys dividend/withholdingTax/coupon/interest/deposit/withdrawal/fxLeg/fee/tradeFee/commissionRefund/capitalReturn/other; free shares received use "shareIn".
