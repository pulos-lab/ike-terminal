...
- "classify": ordered rules, first match wins: {"id","when":[matchers ANDed],"emit":<rowClass>}. Matcher: {"col":<ref>,"op":"equals"|"oneOf"|"contains"|"startsWith"|"regex"|"signPositive"|"signNegative"|"zeroNumber"|"nonzeroNumber"|"empty"|"notEmpty","values"?:[…],"pattern"?:"…"}. Row classes: trade, dividend, withholding_tax, coupon, interest, deposit, withdrawal, fx_leg, fee, trade_fee, commission_refund, capital_return, share_in, redemption, other, skip. "defaultClass": "skip" (unmatched rows reported) or "other" (imported as misc cashflow).
- Every emitted class needs its mapping section: "trade" for trades; cash classes use keys dividend/withholdingTax/coupon/interest/deposit/withdrawal/fxLeg/fee/tradeFee/commissionRefund/capitalReturn/other; free shares received use "shareIn"; redemptions/buyouts use "redemption".
...
- Cash mapping: { "date":{...}, "amount":<vs> (signed), "currency":<vs>, "description"?:<vs>, "ticker"?:<vs>, "isin"?:<vs>, "fxRate"?:<vs>, "fxPair"?:<vs>, "tax"?:<vs>, "taxCurrency"?:<vs> }. When extracting "ticker" from a free-text title with regexExtract, the capture group must match the instrument SYMBOL itself — an UPPERCASE token of 2-8 letters/digits that VARIES between rows (e.g. ELV, AVGO, PLAYWAY) — never a fixed adjacent word like "netto"/"brutto"/"DM". Anchor the pattern on the constant words around the symbol, e.g. "Dywidenda netto ELV 85% USD" → pattern "(?:netto|brutto)\\\\s+([A-Z0-9]{2,8})\\\\b" group 1. Omit "ticker" entirely when no reliable symbol token exists. Use "tax" ONLY when a DIVIDEND row carries its own withholding-tax column on the SAME row — the engine nets it from the amount when same currency (add "taxCurrency" when the tax is in a different currency; then it is noted, NOT subtracted). CRITICAL: if the amount column is already NET of tax (e.g. a "Total" that already deducted WHT), map "amount" to it and DO NOT set "tax" — otherwise tax is subtracted twice. "tax" is mutually exclusive with pairing.dividendWht (that one is for SEPARATE tax rows).
- "shareIn" mapping (free shares RECEIVED with no cash — stock distributions, bonus/grant shares; emits a synthetic buy at price 0, a zero-cost lot): { "date":{...}, "paperName":<vs>, "isin"?:<vs>, "quantity":<vs>, "currency"?:<vs>, "wholeShares"?:bool }. Use it when shares come IN without payment AND (typically) the quantity sits in a DIFFERENT column than ordinary trades. Price is always 0.
- "redemption" mapping (a position CLOSED for cash — bond maturity, tender offer/buyback, certificate redemption; emits a synthetic SELL): { "date":{...}, "paperName":<vs>, "isin"?:<vs>, "quantity":<vs>, "price"?:<vs>, "amount"?:<vs>, "currency"?:<vs>, "wholeShares"?:bool }. Per-share price comes from "price", OR is derived from "amount" (total proceeds) ÷ quantity — set at least one. Use it when a position closes and cash comes IN and the row does NOT fit the standard trade columns (e.g. only a total amount, no per-share price). The file MUST carry the quantity (otherwise classify as skip+corporate_action).
...
- Corporate actions that move share count but do NOT fit the standard trade columns (stock splits, share transfers OUT, reorganizations) often use a DIFFERENT column layout than ordinary trades (e.g. quantity in another column, an id where the price should be). Do NOT force them into "trade"/"other" — that produces garbage. Classify them as skip WITH a reason: {"id":…,"when":[…],"emit":"skip","skipReason":"corporate_action"}. They are reported to the user (not silently dropped) and do not lower confidence. EXCEPTIONS (do NOT skip these): free shares RECEIVED / in-kind stock distributions → "shareIn"; a position CLOSED for cash (redemption/maturity/buyout) WHEN the row carries a quantity → "redemption".
