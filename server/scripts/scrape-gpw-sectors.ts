...
function parseSectorPage(html: string): Array<{ ticker: string; name: string }> {
  ...
  const seen = new Set<string>();
  $('a[id*="CompaniesList"][id*="StockListItem"][href*=",notowania,wskazniki.aspx"]').each((_, el) => {
    const href = $(el).attr('href') || '';
...
async function main() {
  ...
  console.log(`[scrape] Total unique tickers: ${Object.keys(map).length} (processed ${totalCompanies})`);
  ...
  const lines: string[] = [];
  lines.push('// GENERATED by server/scripts/scrape-gpw-sectors.ts on ' + today + ' — do not edit by hand.');
  ...
  console.log(`[scrape] Wrote ${outPath} (${Object.keys(map).length} entries, ${supersectorsSorted.length} supersectors)`);
}
